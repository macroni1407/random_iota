...
contract Pod is ERC721, Ownable{
  ...
  function mint (address _user, Item[] memory _items) external onlyOwner{
    ++itemId;
  ...
  function openPod(uint256 _itemId) external {
    ...
    for(uint256 i; i < retrieveItems.length; ++i){
      ...
      uint256 value = randomNumberGenerator.generateRandomNumberWithLimit(100);
    ...
    console.log("resultItems:", resultItems.length);
  ...
  function getRandomNum() external returns (uint256){
    return randomNumberGenerator.generateRandomNumberWithLimit(100);
  }
}
